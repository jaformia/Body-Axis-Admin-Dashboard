        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-4 px-5 h-12 rounded-[16px] transition-all duration-200 group border ${isActive
            ...
            >
              ...
              />
              <span className={`text-sm font-semibold tracking-wide transition-colors duration-200 ${isActive ? "text-[#2563eb]" : "text-slate-400 group-hover:text-slate-200"
                }`}>
                {item.name}
              </span>
            </Link>
